...
public class LowToHighSort {

    public static List<Book> lowestToHighestPriceSort(List<Book> unsortedBooks, int low, int high){
        if(low < high){
            int indexOfPivotElement = partition(unsortedBooks, low, high);
            lowestToHighestPriceSort(unsortedBooks, low, indexOfPivotElement - 1);
            lowestToHighestPriceSort(unsortedBooks, indexOfPivotElement + 1, high);
        }
